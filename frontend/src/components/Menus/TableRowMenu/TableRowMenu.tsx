...
export default function TableRowMenu({ row, rowIndex }: Props) {
	const [menuOpened, toggle] = useToggle([false, true])
	const { addNewLine } = useContext(RowsContext)
	return (
		<Menu transitionProps={{ duration: 0 }} opened={menuOpened} onChange={toggle}>
			<Menu.Target>
			...
			<Menu.Dropdown>
				...
				<Menu.Item icon={<IconPlus size={rem(20)} />}>Add Row Above</Menu.Item>
				<Menu.Item icon={<IconPlus size={rem(20)} />}>Add Row Below</Menu.Item>
